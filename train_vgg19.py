...
# with tf.device('/cpu:0'):
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=True)) as sess:
        ...
        try:
            vgg = vgg19.Vgg19('Weights/VGG19_{}C.npy'.format(classes_num),classes_num)
        except:
            print('Weights/VGG19_{}C.npy Not Exist'.format(classes_num))
        ...
        for _ in range(epochs):
                ...
                for i in range(int(batche_num/batch_size)):
                    ...
                    if counter % 100 == 0:
                        #  save graph data
                        append(costs,'Data/cost19_{}C.txt'.format(classes_num))
                        append(accs,'Data/acc19_{}C.txt'.format(classes_num))
                        # save Weights
                        vgg.save_npy(sess, 'Weights/VGG19_{}C.npy'.format(classes_num))
